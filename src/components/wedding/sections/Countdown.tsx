...
function Unit({ value, label }: { value: number; label: string }) {
  return (
    <motion.div
      ...
      className="relative grid aspect-square place-items-center rounded-2xl glass-gold p-3 shadow-gold sm:rounded-3xl"
      data-cursor
    >
      <span className="pointer-events-none absolute inset-0 rounded-2xl border border-gold/30 sm:rounded-3xl" />
      <span className="font-display text-3xl font-semibold tabular-nums text-maroon sm:text-5xl">
        {String(value).padStart(2, "0")}
      </span>
      <span className="mt-1 font-sans text-[10px] uppercase tracking-[0.25em] text-gold-deep sm:text-xs">{label}</span>
    </motion.div>
  );
}
